...
gesture_detector.py
────────────────────
Enhanced MediaPipe Hands detector built on top of the existing pipeline.

Key additions over the original mediapipe_detection.py:
  - Finger count extracted per hand on every frame (used for tab-switch modifier)
  - Palm velocity tracking per hand (used for two-hand multiplier hold detection)
  - Clean dataclass output instead of raw dicts
  - Config-driven confidence thresholds (reads from ConfigManager)
  - Pinch distance exposed for ghost cursor layer
  - All original logic (angle-based extension, palm facing, dynamic swipe/wave) preserved

Output per frame: FrameResult dataclass consumed by GestureRouter.
"""
...
class GestureDetector:
    ...
    # MediaPipe landmark indices (kept as instance attrs to match original style)
    WRIST = 0
    ...
    def __init__(self, config: ConfigManager):
        ...
        # Two-hand combo patterns (from original, driven by config bindings)
        self._combo_patterns = self._build_combo_patterns()

        # Register reload callback so patterns update if config changes
    ...
    def _build_combo_patterns(self):
        ...
        for gesture_id, g in self.gestures.items():
            
            # FIX: Check if g is actually a dictionary before calling .get()
            if not isinstance(g, dict):
                continue 
                
            if g.get("type") == "combo" and "combo_rule" in g:
                # Your existing logic to build patterns
                combo_patterns[gesture_id] = g.get("combo_rule")
    ...
    # ── Finger Extension (angle-based, from original) ──────────────────────────
    ...
    # ── Static Gesture Detection (from original, extended) ────────────────────
    ...
    # ── Dynamic Gesture Detection (from original) ─────────────────────────────
