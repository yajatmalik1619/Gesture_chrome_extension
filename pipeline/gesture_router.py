...
Responsibilities:
  ...
  3. Implement the two-hand multiplier mechanic:
       - Track which hand has been stationary for ≥ hold_duration_seconds
    ...
    def update(self, label: str, is_stationary: bool, finger_count: int) -> Optional[int]:
        ...
        if is_stationary:
            ...
            if elapsed >= self.hold_duration and self._multiplier[label] is None:
                ...
                logger.info(
                    f"Multiplier locked: {label} hand → {self._multiplier[label]} fingers"
                )
        else:
            # Hand moved — clear multiplier and timer
...
class GestureRouter:
    """
    Converts FrameResult → list[ActionEvent].
    ...
    def route(self, frame_result: FrameResult) -> list[ActionEvent]:
        ...
        # ── Update multiplier tracker for all detected hands ──────────────────
    ...
    def _resolve_hand(self, hr: HandResult, frame: FrameResult) -> Optional[ActionEvent]:
        ...
        # If no built-in binding, try DTW custom gesture match
        if not action_id:
            if hr.dynamic_gesture:
                custom_match = self.dtw.match_dynamic(
                    self._get_dynamic_sequence(hr)
                )
            else:
        ...
        if gesture_id in self.TAB_SWIPE_GESTURES and self.cfg.action_has_modifier(action_id):
            ...
            logger.info(
                f"Tab swipe: {hr.label} hand, {swipe_fingers}f × {multiplier_fingers}x = {magnitude} tabs"
            )

        # ── Extra metadata for cursor layer ──────────────────────────────────
        meta = {}
        if action_id.startswith("cursor"):
    ...
    @staticmethod
    def _get_dynamic_sequence(hr: HandResult) -> list:
        """
        Placeholder — in a full implementation the detector would buffer
        the recent landmark frames for dynamic DTW. For now returns a
        single-frame sequence so the call path is valid.
        """
        return [hr.landmarks]
